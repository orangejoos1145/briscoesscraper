...
# This dictionary maps keywords found in the granular category to a "Super Section".
# You can add more keywords to these lists to refine the grouping.
CATEGORY_MAPPINGS = {
    "Kitchen & Cooking": [
...
def get_super_category(raw_cat):
    """
    Takes a granular category (e.g. 'Air Fryers') and returns a Super Category (e.g. 'Kitchen').
    """
    if pd.isna(raw_cat): 
        return "Other"
...
def generate_category_filters_html(cat_list):
    ...
    # Sort: Put 'Other / Brands' last, everything else alphabetical
...
try:
    ...
    print(f"Error: Input file '{IN_CSV}' not found. Creating empty placeholder.")
...
for idx, row in df.iterrows():
    ...
    # --- CATEGORY LOGIC ---
    # 1. Get the raw category string (e.g., "Air Fryers;;Sale")
    raw_cat_str = str(row.get("Category", "Other"))
    if pd.isna(raw_cat_str) or raw_cat_str.lower() == "nan":
        raw_cat_str = "Other"
    
    # 2. Extract the first meaningful tag from the ;; string for "Badge" logic
    # (We keep this for searching, but use Super Category for filtering)
    specific_category = raw_cat_str.split(';;')[0].strip()
    
    # 3. Determine the Super Category
    ...
    deals_payload.append({
        ...
        "c": super_category, # We store the Super Category here for the Filter Buttons
        "sc": specific_category # We store the Specific Category for the display badge
    })
...
if os.path.exists(WHATS_NEW_FILE):
    try:
        ...
        whats_new_content = f"Error reading whatsnew: {e}"
...
<head>
<meta charset="utf-8"/>
<title>Briscoes Deals Filterer</title>
...
<style>
  :root {{ 
    /* BRISCOES THEME COLORS */
    --accent: #004B8D;        /* Briscoes Navy Blue */
    --accent-dark: #003366; 
    --highlight: #FFCE00;     /* Briscoes Yellow */
    ...
    --row-hover: #eef1f5;
    --btn-text: #fff;
    --btn-highlight-text: #222; /* Text on yellow buttons */
  }}
  :root.dark {{ 
  ...
  @media (max-width: 768px) {{
    ...
    thead th:nth-child(1), tbody td:nth-child(1) {{ display: none; }} /* Hide ID */
    thead th:nth-child(3), tbody td:nth-child(3) {{ display: none; }} /* Hide Original Price */
...
function renderPage() {{
    ...
    slice.forEach(d => {{
        ...
        if (d.l && d.l !== '#') {{
             linkHtml = `<a class="product-link" href="${{d.l}}" target="_blank">${{escapeHtml(d.n)}}</a>`;
        }}
        
        let pctDisplay = '';
        if (d.v > 0) {{
            pctDisplay = `${{Math.round(d.v)}}%`;
        }}

        // NOTE: We display d.sc (specific category) in the table so user sees "Air Fryer"
        // But the FILTERS use d.c (Super Category)
        html += `<tr>
            <td style="font-family:monospace; color:var(--muted); font-size:12px;">${{escapeHtml(d.p)}}</td>
            <td>${{linkHtml}}</td>
            <td class="price" style="text-decoration:line-through; color:var(--muted);">${{d.o}}</td>
            <td class="price" style="font-weight:bold;">${{d.d}}</td>
            <td class="discount">${{pctDisplay}}</td>
            <td><span style="background:var(--row-hover); padding:2px 8px; border-radius:4px; font-size:12px; white-space:nowrap;">${{escapeHtml(d.sc)}}</span></td>
            <td style="text-align:center;"><a href="${{googleLink}}" target="_blank">${{googleIconSvg}}</a></td>
        </tr>`;
    }});
    
    if (slice.length === 0) {{
        html = '<tr><td colspan="7" style="text-align:center; padding:20px;">No deals found matching filters.</td></tr>';
    }}
...
function applyFilters() {{
    ...
    state.filtered = allData.filter(d => {{
        // Filter checks against the SUPER category (d.c)
        if (state.activeCategory !== 'all') {{
            if (!d.c || d.c.toLowerCase() !== state.activeCategory) return false;
        }}
        if (state.hideZero && d.v <= 0) return false;
        if (d.v < state.minPct || d.v > state.maxPct) return false;
        
        // Search checks title, specific category, and ID
        if (term) {{
            const haystack = (d.n + ' ' + d.sc + ' ' + d.p).toLowerCase();
...
function setupListeners() {{
    ...
    document.querySelectorAll('.cat-filter-btn, [data-cat="all"]').forEach(btn => {{
        btn.addEventListener('click', (e) => {{
            document.querySelectorAll('.cat-filter-btn, [data-cat="all"]').forEach(b => b.classList.remove('active'));
    ...
    document.getElementById('resetBtn').addEventListener('click', () => {{
        ...
        document.getElementById('minPct').value = 0;
        document.getElementById('maxPct').value = 100;
    ...
    document.getElementById('btnNext').addEventListener('click', () => {{
        const max = Math.ceil(state.filtered.length / state.rowsPerPage);
        if(state.currentPage < max) {{ state.currentPage++; renderPage(); }}
    }});
